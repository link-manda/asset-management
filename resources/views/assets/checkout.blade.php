            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('assets.checkout', $asset) }}" method="POST">
                        ...
                        <div class="mb-5">
                            <label class="inline-block mb-2 text-sm text-default-800 font-medium" for="assigned_to">Berikan Kepada (User)</label>
                            <select class="form-select @error('assigned_to') border-danger @enderror" id="assigned_to" name="assigned_to" required>
                                <option value="" disabled selected>-- Pilih User Penerima --</option>
                        ...
                        <div class="mb-5">
                            <label class="inline-block mb-2 text-sm text-default-800 font-medium" for="assigned_date">Tanggal Penugasan</label>
                            <input class="form-input" id="assigned_date" name="assigned_date" type="date" value="{{ old('assigned_date', date('Y-m-d')) }}" required />
                        </div>

                        <div class="mb-5">
                            <label class="font-medium text-default-800 text-sm mb-2 inline-block" for="condition_on_checkout">Kondisi Saat Keluar</label>
                            <textarea class="form-input @error('condition_on_checkout') border-danger @enderror" id="condition_on_checkout" name="condition_on_checkout" placeholder="Jelaskan kondisi fisik aset saat diserahkan..." rows="5" required>{{ old('condition_on_checkout', 'Normal / Baik') }}</textarea>
                    ...
                    <div class="flex gap-3">
                        ...
                        <div>
                            <p class="text-xs text-default-600 leading-relaxed font-medium">
                                Pastikan User menandatangani berita acara serah terima aset setelah penugasan ini dikonfirmasi.
                            </p>
                        </div>
                    </div>
                </div>
